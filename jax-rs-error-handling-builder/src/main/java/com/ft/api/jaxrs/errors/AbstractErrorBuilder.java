...
public abstract class AbstractErrorBuilder<E, B extends AbstractErrorBuilder> {
  ...
  /**
   * Used with enumerated reason code systems. Extracts a message from the enum per <code>
   * error(enumeratedReason.toString())</code>
   *
   * <p>This is used where the business wants to sign off on and control messages, and allows
   * developers to consolidate signed off messages in an enum with it's own Javadoc etc. and manage
   * the messages in source control.
   *
  ...
  public Response response() {
    ...
    // This is not a server error and the serverError method is used as a template to build a
  ...
  /**
   * Creates an exception wrapping the configured response and the <code>cause</code>. The exception
   * implements a Jersey exception type and contains an HTTP response code and body, and therefore
   * any configured exception mapping is skipped including {@link RuntimeExceptionMapper}.
   *
   * @param cause
  ...
  /**
   * Creates an exception wrapping the configured response. The exception implements a Jersey
   * exception type and contains an HTTP response code and body, and therefore any configured
   * exception mapping is skipped including {@link RuntimeExceptionMapper}.
   *
   * @return an exception, which should be thrown.
   */
